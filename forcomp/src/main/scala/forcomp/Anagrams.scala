...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toSeq.groupBy(c => c).map {
      case (k, v) => (k, v.length)
    }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def extract(occurrences: Occurrences): List[Occurrences] = {
      occurrences match {
        case Nil => List(Nil)
        case h :: t =>
          for {
            subCombination <- extract(t)
            freq <- 0 to h._2
            occur = if (freq == 0) Nil else List((h._1, freq))
          } yield {
            occur ++ subCombination
          }
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for {
      xSub <- x
      filtered = yMap get xSub._1 match {
        case None => xSub
        case Some(i) => (xSub._1, xSub._2 - i)
      }
      if filtered._2 > 0
    } yield {
      filtered
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def extract(occurrences: Occurrences): List[Sentence] = {
      occurrences match {
        case Nil => List(Nil)
        case _ =>
          for {
            subsets <- combinations(occurrences)
            words <- dictionaryByOccurrences get subsets match {
              case Some(w) => w
              case None => Nil
            }
            others <- extract(subtract(occurrences, subsets))
          } yield words :: others
      }
    }

    extract(sentenceOccurrences(sentence))
  }
}
