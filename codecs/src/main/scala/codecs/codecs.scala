...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction { case Json.Arr(as) => as.flatMap(_.decodeAs[A]) }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction { case Json.Obj(obj) if obj.contains(name) => obj(name).decodeAs[A].get }
}
