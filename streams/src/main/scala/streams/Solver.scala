...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    ...
    else for (neighbor <- neighbors.to(LazyList)) yield (neighbor._1, neighbor._2 :: history)
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block])
  : LazyList[(Block, List[Move])] = for (neighbor <- neighbors if explored.contains(neighbor._1) == false) yield neighbor
  ...
  : LazyList[(Block, List[Move])] = {
    if (initial.isEmpty) LazyList.empty
      ...
      val paths = for {
        seq <- initial
        next <- newNeighborsOnly(neighborsWithHistory(seq._1, seq._2), explored)
      } yield next
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.map(_._2).minBy(_.length).reverse
}
