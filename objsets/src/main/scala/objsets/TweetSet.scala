...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val others = remove(elem)
    try {
      if (elem.retweets > others.mostRetweeted.retweets)
        elem
      else
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.contains(tweet))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.contains(tweet))
